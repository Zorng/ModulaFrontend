...
class _EditableCompositionRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final selectedStockItemId = stockItems.any(
      (item) => item.id == row.selectedStockItemId,
    )
        ? row.selectedStockItemId
        : null;
    final selectedTrackingMode =
        row.trackingMode == 'UNTRACKED' ? 'UNTRACKED' : 'TRACKED';

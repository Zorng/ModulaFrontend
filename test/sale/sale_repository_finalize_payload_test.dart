...
void main() {
  ...
  test(
    'placeOrder still reaches the API for manual-claim orders even when cached policy is false',
    () async {
      final api = _MockSaleApi();
      final repository = SaleRepository(
        api,
      ...
      await repository.placeOrder(
        const SalePlaceOrderCommand(
          saleId: 'local-claim-1',
          branchId: 'branch-1',
          saleType: 'take_away',
          clientOpId: 'place-manual-claim-1',
          sourceMode: 'MANUAL_EXTERNAL_PAYMENT_CLAIM',
          cartLines: [
            SaleCartLineInputDto(
              menuItemId: 'item-claim-1',
              quantity: 1,
              modifiers: [],
            ),
          ],
  ...
  test(
    'finalizeSale calls KHQR confirm fallback when intent is paid but sale is not materialized yet',
    () async {
      final api = _MockSaleApi();
      final repository = SaleRepository(api);

      when(() => api.getKhqrIntentStatus('intent-2')).thenAnswer(
        (_) async => const SaleKhqrIntentStateDto(
      ...
      when(
        ...
        (_) async => SaleKhqrConfirmResponseDto.fromJson({
      ...
      when(() => api.getSaleDetail('sale-2')).thenAnswer(
        (_) async => SaleDto.fromJson({
          'id': 'sale-2',
          'orderId': 'order-2',
          'clientUuid': 'client-2',
      ...
      when(() => api.getReceiptBySaleId('sale-2')).thenAnswer(
        (_) async => SaleReceiptReadDto.fromJson({
          'receiptId': 'receipt-2',
          'saleId': 'sale-2',
          'receiptNumber': 'R-2',
      ...
      final result = await repository.finalizeSale(
        const SaleFinalizeSaleCommand(
          ...
          paymentMethod: 'khqr',
          tenderCurrency: 'USD',
          clientOpId: 'client-op-khqr-2',
          khqrIntentId: 'intent-2',
          khqrMd5: 'khqr-md5-2',
        ),
      );

      expect(result.saleId, 'sale-2');
      verify(() => api.getKhqrIntentStatus('intent-2')).called(1);
      verify(
        () => api.confirmKhqrPayment(
          'khqr-md5-2',
          idempotency: any(named: 'idempotency'),
      ...
      verify(() => api.getSaleDetail('sale-2')).called(1);
    },
  );

  test(
    'finalizeSale returns KHQR finalization pending when confirm fallback does not materialize a sale',
    () async {
      final api = _MockSaleApi();
      final repository = SaleRepository(api);

      when(() => api.getKhqrIntentStatus('intent-3')).thenAnswer(
        (_) async => const SaleKhqrIntentStateDto(
      ...
      when(
        ...
        (_) async => const SaleKhqrConfirmResponseDto(
          verificationStatus: 'CONFIRMED',
          saleFinalized: false,
        ),
      );
      ...
      await expectLater(
        repository.finalizeSale(
          const SaleFinalizeSaleCommand(
            ...
            paymentMethod: 'khqr',
            tenderCurrency: 'USD',
            clientOpId: 'client-op-khqr-3',
            khqrIntentId: 'intent-3',
            khqrMd5: 'khqr-md5-3',
          ),
        ),
        throwsA(
          isA<SaleCheckoutRepositoryException>()
              .having(
                (e) => e.reasonCode,
                'reasonCode',
                SaleCheckoutReasonCodes.khqrFinalizationPending,
              )
              .having(
                (e) => e.message,
                'message',
                contains('backend finalization is still pending'),
              ),
        ),
      );

      verify(() => api.getKhqrIntentStatus('intent-3')).called(1);
      verify(
        () => api.confirmKhqrPayment(
          'khqr-md5-3',
          idempotency: any(named: 'idempotency'),
      ...
      verifyNever(() => api.getSaleDetail(any()));
    },
  );
